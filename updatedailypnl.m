function updatedailypnl(pathfilewm, varargin)
tic
%% Individual Analysis
...
if ~exist(char(strcat(pathfilewm, '\', 'Portfolio_PNL','_', ...
    ...
    % add cash
    cash = readtable(xlsfileaccpath, 'filetype', ...
        'spreadsheet', 'sheet', 3);
    cashCA = readtable(fullfile(pathfilewm, 'hExpenses'), 'filetype', ...
        'spreadsheet', 'sheet', 1, 'range','E1:E2');
    cashSA = readtable(fullfile(pathfilewm, 'Account&PNL'), 'filetype', ...
        'spreadsheet', 'sheet', 2, 'range','F:F');
    cashTable = table();
    accounts = [strcat('Cash_',cash.CashAccount); ...
        {'Cash_Current_Account'; 'Cash_Saving_Account'}];
    cashTable.AssetType = accounts;
    cashTable.ValueAccAssetType = [cash.Value; cashCA.SALDO; ...
        cashSA.mean_SavingAccount(end)];
    cashTable.PercbyAssetType = 100*(cashTable.ValueAccAssetType ./ ...
        sum(cashTable.ValueAccAssetType));
    cashTable.ValuetAccAssetType = cashTable.ValueAccAssetType;
    cashTable.RetbyAssetType = cashTable.ValuetAccAssetType ./...
        cashTable.ValueAccAssetType;
    % join with asset type table
    tblAssetType = [tblAssetType;cashTable];
    ...
    cashagg = tblExch;
    cashagg(2,:) = [];
    cashagg(:,1) = {'Total_Cash'};
    cashagg(:,2) = array2table(sum(cashTable.ValueAccAssetType));
    cashagg(:,3) = array2table(1);
    cashagg(:,4) = array2table(sum(cashTable.ValueAccAssetType));
    cashagg(:,5) = array2table(1);
    % join with asset type table
    tblExch = [tblExch; cashagg];
    ...
    tickhold = [tbl.UniqueSymbol(idx); 'ALVG']; % patch
    ...
    %% DPY in cryptocurrencies
    % first check that the portfolio have cryptos
    isCrypto = ismember(dailyinfoportfh.Category1,'Crypto');
    isCryptoAlive = sum(dailyinfoportfh.shares(isCrypto > 0));
    if isCryptoAlive > 0
        disp(['Obtaining DPY Cryptos at ', ...
            char(datetime('now','format','HH:mm'))])
        % obtain shares in each cryptos
        CryptoTable = groupsummary(dailyinfoportfh(isCrypto, {'Name',...
            'shares'}), 'Name');
        % need info about interest rate, start date,...
        [~,nsheetsacc] = xlsfinfo(xlsfileaccpath);
        if sum(strcmp(nsheetsacc, 'APY_Cryptos')) == 0
            error('APY_Cryptos does not exist!')
        end
        APYCrypto =  readtable(xlsfileaccpath, 'filetype', ...
            'spreadsheet', 'sheet', 'APY_Cryptos');
        % join info
        CryptoTable = join(CryptoTable,  APYCrypto);
        CryptoTable.Properties.RowNames = CryptoTable.Name;
        % initial APY criptos
        initdates = min(unique(CryptoTable.InitDate));
        DPY_Crypto = timetable();
        for k = 1:height(CryptoTable)
            ticker =  matlab.lang.makeValidName(CryptoTable.Name(k));
            path2search = fullfile(pathfilewm, ticker);
            file = fullfile(path2search,[char(ticker), '_', ...
                datestr(today()),'.xlsx']);
            % get info from the ticker
            infoTable = readtimetable(char(file),'filetype', ...
                'spreadsheet', 'sheet', 1);
            % filter the table
            infoTableFilter = infoTable(infoTable.Time >= initdates, ...
                [strcat('Hist_', ticker),strcat('Shares_', ticker)]);
            % change first share (needeed with btc y eth)
            infoTableFilter(initdates, strcat('Shares_', ticker)) = ...
                CryptoTable(CryptoTable.Name(k),'InitPos');
            % DPY
            infoTableFilter.DPY = repmat(CryptoTable{CryptoTable.Name(k),'DPY'},...
                height(infoTableFilter), 1) .* infoTableFilter{:, strcat('Shares_', ticker)}...
                .* infoTableFilter{:,strcat('Hist_', ticker)};
            % Acc DPY
            infoTableFilter.AccDPY = cumsum(infoTableFilter.DPY);
            infoTableFilter.Properties.VariableNames(3) = strcat('DPY_', ticker);
            infoTableFilter.Properties.VariableNames(4) = strcat('AccDPY_', ticker);
            DPY_Crypto = [DPY_Crypto, infoTableFilter];
            % update cryptos
            % check first the frencuency
            frecuency = CryptoTable.tFrecuency(k);
            if strcmp(frecuency,'daily') && CryptoTable.nFrecuency(k) > 0
                disp(['Updating new positions in Cryptos: ', char(ticker),...
                    ' at ', char(datetime('now','format','HH:mm'))])
                % get a row in account table
                dailyinfoportf =  readtable(xlsfileaccpath, 'filetype', ...
                    'spreadsheet', 'sheet', 1);
                checkCrypto = ismember(matlab.lang.makeValidName(dailyinfoportf.symbol), ticker);
                crypto = dailyinfoportf(checkCrypto > 0, :);
                % get not buyed cryptos
                crypto = crypto(crypto.loc_quantity == 0, :);
                cryptoEx = crypto(end,:);
                % change date, price, shares and currency
                cryptoEx.Date = datetime('today','format','MM/dd/yyyy');
                cryptoEx.price = DPY_Crypto{cryptoEx.Date,strcat('Hist_', ticker)};
                cryptoEx.shares = DPY_Crypto{cryptoEx.Date,strcat('DPY_', ticker)}...
                    / DPY_Crypto{cryptoEx.Date,strcat('Hist_', ticker)};
                if strcmp(cryptoEx.currency, 'EUR')
                    cryptoEx.fx = 1;
                else
                    cryptoEx.fx = AllHistoricalTable{cryptoEx.Date,...
                        'USD_EUR_USDollarEuro'};
                end
                % store shares in account
                pos_cryptos_update = [dailyinfoportf; cryptoEx];
                writetable(pos_cryptos_update,xlsfileaccpath, 'filetype', ...
                    'spreadsheet', 'sheet', 1);
                disp(['Updated new positions in Cryptos: ', char(ticker),...
                    ' at ', char(datetime('now','format','HH:mm'))])
            elseif strcmp(frecuency,'ndaily') && CryptoTable.nFrecuency(k) > 0
                % the same code as above but delaing with "n" days of
                % frencuency
                dailyinfoportf =  readtable(xlsfileaccpath, 'filetype', ...
                    'spreadsheet', 'sheet', 1);
                checkCrypto = ismember(matlab.lang.makeValidName(dailyinfoportf.symbol), ticker);
                ...
                crypto = crypto(crypto.loc_quantity == 0, :);
                cryptoEx = crypto(end,:);
                cryptoStoreEx = cryptoEx;
                % check the frencuency
                daysFrecuency = CryptoTable.nFrecuency(k);
                % only store the money earned if it is a "pay day"
                if datetime('today','format','MM/dd/yyyy') - ...
                        cryptoEx.Date == daysFrecuency
                    disp(['Updating new positions in Cryptos: ', char(ticker),...
                        ' at ', char(datetime('now','format','HH:mm'))])
                    % change date, price, shares and currency
                    cryptoStoreEx.Date = datetime('today','format','MM/dd/yyyy');
                    cryptoStoreEx.price = DPY_Crypto{cryptoEx.Date,strcat('Hist_', ticker)};
                    % sum dpy after the last pay day 
                    datesToSum = DPY_Crypto.Time > cryptoEx.Date;
                    DpyInThatPeriod = DPY_Crypto{datesToSum, strcat('DPY_', ticker)};
                    cryptoStoreEx.shares = sum(DpyInThatPeriod) / cryptoStoreEx.price;
                    % change cuurency
                    if strcmp(cryptoEx.currency, 'EUR')
                        cryptoStoreEx.fx = 1;
                    else
                        cryptoStoreEx.fx = AllHistoricalTable{cryptoStoreEx.Date,...
                            'USD_EUR_USDollarEuro'};
                    end
                    % store new shares in account
                    pos_cryptos_update = [dailyinfoportf; cryptoStoreEx];
                    writetable(pos_cryptos_update,xlsfileaccpath, 'filetype', ...
                        'spreadsheet', 'sheet', 1);
                    disp(['Updated new positions in Cryptos: ', char(ticker),...
                        ' at ', char(datetime('now','format','HH:mm'))])
                end
            elseif strcmp(frecuency,'monthly') && CryptoTable.nFrecuency(k) > 0
                % the same code as above but delaing with monthly days of
                % frencuency
                if datestr(today()) == dateshift(datetime(datestr(today())), 'end', 'month')
                    disp(['Updating new positions in Cryptos: ', char(ticker),...
                        ' at ', char(datetime('now','format','HH:mm'))])
                    ...
                    crypto = crypto(crypto.loc_quantity == 0, :);
                    cryptoEx = crypto(end,:);
                    cryptoEx.Date = datetime('today','format','MM/dd/yyyy') + 1;
                    cryptoEx.price = DPY_Crypto{datetime('today','format','MM/dd/yyyy'),...
                        strcat('Hist_', ticker)};
                    % groupsummary to get the sum of interest in the month
                    dpy_month = groupsummary(DPY_Crypto,'Time','month', 'sum',...
                        char(strcat('DPY_', ticker)));
                    dpy_month.month_Date = dateshift(datetime(cellstr(dpy_month.month_Time),...
                        'format','dd-MM-yyyy'),'end', 'month');
                    dpy_month_tt = table2timetable(dpy_month);
                    dpy_month_tt.GroupCount = [];
                    % calculate shares with the interest in the month
                    cryptoEx.shares = dpy_month_tt{dateshift(datetime('today',...
                        'format', 'dd-MM-yyyy'),'end','month'), ...
                        strcat('sum_DPY_', ticker)} / cryptoEx.price;
                    if strcmp(cryptoEx.currency, 'EUR')
                        cryptoEx.fx = 1;
                    else
                        cryptoEx.fx = AllHistoricalTable{cryptoEx.Date,...
        ...
        % get the accumulated value in interest rate money
        names2filterAcc = startsWith(DPY_Crypto.Properties.VariableNames,...
            'AccDPY_');
        names2filterAcc = DPY_Crypto.Properties.VariableNames(names2filterAcc);
        AccallData = DPY_Crypto{:, names2filterAcc};
        DPY_Crypto.SumAccCrypto = sum(AccallData,2, 'omitnan');
    end
    %% plot
    ...
    ewb.Worksheets.Item(6).Name  = 'DPY_Crypto';
    ewb.Worksheets.Item(7).Name  = 'VaR_and_ES_Simulated_detail';
    ewb.Worksheets.Item(8).Name  = 'PNL_CategoryAsset'; 
    % save to the same file
    ewb.Save
    ewb.Close(true)
    e.Quit
